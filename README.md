...
> **Note:** `data/processed/state_pops.rds` is not included in this repository — it is generated by `01_prepare_data.R` and excluded from version control due to size. The `st_intersection()` clipping step is the main bottleneck: It takes approximately 12-14 hours on an M1 Max MacBook Pro. The result is cached so you only need to do this once.
